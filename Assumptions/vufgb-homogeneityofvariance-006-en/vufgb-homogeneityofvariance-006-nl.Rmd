```{r, echo = FALSE, results = "hide"}
include_supplement("vufgb-homogeneityofvariance-006-en-table01.jpg", recursive = TRUE)
```

Question
========

See below some data from a study of abstract reasoning in three different groups. What can we infer from this data? 

Complete.

The ... of the three groups are significantly different, the assumption of ... is violated.

![](vufgb-homogeneityofvariance-006-en-table01.png)
  
Answerlist
----------
* means; normality
* means; independent observations
* variances; homogeneity
* variances; multicollinearity

Solution
========

Answerlist
----------
* Incorrect
* Incorrect
* Correct
* Incorrect

Meta-information
================
exname: vufgb-homogeneityofvariance-006-en
extype: schoice
exsolution: 0010
exsection: Assumptions/Homogeneity of variance
exextra[Type]: Interpreting output
exextra[Program]: 
exextra[Language]: English
exextra[Level]: Statistical Literacy
