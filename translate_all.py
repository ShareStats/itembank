...
def translate_folder(deepl_client, input_folder,output_folder,target_glossaries):
...
if __name__ == "__main__":
    ...
    if not deepl_api_key:
        raise ValueError("Please set DEEPL_API_KEY environment variable before running the script.")
    ...
    folders = []    
    for item_folder in base_folders:
        folders+=[folder for folder, _ ,file in os.walk(item_folder) if file]

    folder_stripped = [folder[:-3] for folder in folders]
    folders_n = Counter(folder_stripped) 

    for folder in folders:
        if folders_n[folder[:-3]] == 1:
            print(f'TRANSLATING: {folder}')
            translate_folder(deepl_client=deepl_client,input_folder=folder, output_folder='./',target_glossaries=target_glossaries)
